...
class group_lasso_Dense(Module):
	...
	def __init__(self, in_features, out_features, bias=True, lamba=1., weight_decay=1., **kwargs):
		...
		"""
		super(group_lasso_Dense,self).__init__()
	...
	def _reg_w(self, **kwargs):
		logpw = -self.lamba*np.sqrt(self.out_features)*torch.sum(torch.pow(torch.sum(self.weight.pow(2),1),0.5))
		logpb = 0
		if self.bias is not None:
...
class group_lasso_Conv2d(Module):
	...
	def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True,
		...
		"""
		super(group_lasso_Conv2d, self).__init__()
		self.floatTensor = torch.FloatTensor if not torch.cuda.is_available() else torch.cuda.FloatTensor
	...
	def _reg_w(self, **kwargs):
		logpw = -self.lamba*np.sqrt(self.in_channels*self.kernel_size[0]*self.kernel_size[1])*torch.sum(torch.pow(torch.sum(self.weight.pow(2),3).sum(2).sum(1),0.5))
		logpb = 0
		if self.bias is not None:
			logpb = - torch.sum(self.weight_decay * .5 * (self.bias.pow(2)))
		return logpw+logpb
