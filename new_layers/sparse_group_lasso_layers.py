...
class sparse_group_lasso_Dense(Module):
	"""Implementation of TFL regularization for the input units of a fully connected layer"""
	def __init__(self, in_features, out_features, bias=True, lamba=1., weight_decay=1., **kwargs):
		"""
		...
		"""
		super(sparse_group_lasso_Dense,self).__init__()
	...
	def _reg_w(self, **kwargs):
		logpw = -self.lamba*np.sqrt(self.out_features)*torch.sum(torch.pow(torch.sum(self.weight.pow(2),1),0.5))-torch.sum(self.lamba*self.weight.abs())
		logpb = 0
		if self.bias is not None:
...
class sparse_group_lasso_Conv2d(Module):
	...
	def _reg_w(self, **kwargs):
		logpw = -self.lamba*np.sqrt(self.in_channels*self.kernel_size[0]*self.kernel_size[1])*torch.sum(torch.pow(torch.sum(self.weight.pow(2),3).sum(2).sum(1),0.5))-torch.sum(self.lamba*self.weight.abs())
		logpb = 0
		if self.bias is not None:
			logpb = - torch.sum(self.weight_decay * .5 * (self.bias.pow(2)))
		return logpw+logpb
