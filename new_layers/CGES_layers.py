...
class CGES_Dense(Module):
	...
	def __init__(self, in_features, out_features, bias=True, lamba=1., weight_decay=1., mu = 0.5, **kwargs):
		"""
	...
	def _reg_w(self, **kwargs):
		logpw = -(1-self.mu)*self.lamba*torch.sum(torch.pow(torch.sum(self.weight.pow(2),1),0.5))-self.mu*self.lamba/2*torch.sum(torch.sum(self.weight.abs(),1)**2)
		logpb = 0
		if self.bias is not None:
...
class CGES_Conv2d(Module):
	...
	def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True,
		...
		"""
		super(CGES_Conv2d, self).__init__()
		self.floatTensor = torch.FloatTensor if not torch.cuda.is_available() else torch.cuda.FloatTensor
	...
	def _reg_w(self, **kwargs):
		logpw = -self.lamba*(1-self.mu)*torch.sum(torch.pow(torch.sum(self.weight.pow(2),3).sum(2).sum(1),0.5))-self.lamba/2*self.mu*torch.sum(torch.pow(torch.sum(self.weight.abs(),3).sum(2).sum(1),2))
		logpb = 0
		if self.bias is not None:
			logpb = - torch.sum(self.weight_decay * .5 * (self.bias.pow(2)))
		return logpw+logpb
