...
class L0Dense(Module):
    ...
    def count_active_neuron(self):
        #return torch.sum(torch.sum(self.weights.abs()/self.out_features,1)>1e-5).item() 
        return self.ppos.item()

    def count_total_neuron(self):
        return self.in_features

    def count_zero_w(self):
        #return torch.sum((self.weights.abs()<1e-5).int()).item()
        count = self.ppos*self.out_features
        return count.item()

    def count_weight(self):
        return np.prod(self.weights.size())
    ...
    def count_expected_flops_and_l0(self):
        ...
        self.ppos = torch.sum(1 - self.cdf_qz(0))
        expected_flops = (2 * self.ppos - 1) * self.out_features
        expected_l0 = self.ppos * self.out_features
        if self.use_bias:
...
class L0Conv2d(Module):
    ...
    def count_active_neuron(self):
        #return torch.sum((torch.sum(self.weights.abs(),3).sum(2).sum(1)/(self.in_channels*self.kernel_size[0]*self.kernel_size[1]))>1e-5).item()
        return self.ppos.item()
    ...
    def count_expected_flops_and_l0(self):
        ...
        expected_flops = flops_per_filter * self.ppos  # multiply with number of filters
        expected_l0 = n * self.ppos

        if self.use_bias:
            # since the gate is applied to the output we also reduce the bias computation
            expected_flops += num_instances_per_filter * self.ppos
            expected_l0 += self.ppos
