...
parser.add_argument('--name', default='group_relaxed_L1L2WideResNet', type=str,
...
parser.add_argument('--lamba', type=float, default=0.001,
                    help='Coefficient for the transformed L1 term.')
parser.add_argument('--alpha', type=float, default=1.0,
                    help='parameter for transformed L1')
...
def main():
    ...
    # create model
    if args.reg=='l0':
    	model = group_relaxed_L0WideResNet(args.depth, num_classes, widen_factor=args.width, droprate_init = args.droprate_init,
    		N=50000, beta_ema=args.beta_ema, weight_decay=args.weight_decay, lamba=args.lamba, beta=args.beta)
    elif args.reg=='l1':
    	model = group_relaxed_L1WideResNet(args.depth, num_classes, widen_factor=args.width, droprate_init = args.droprate_init,
    		N=50000, beta_ema=args.beta_ema, weight_decay=args.weight_decay, lamba=args.lamba, beta=args.beta)
    elif args.reg=='l1l2':
    	model = group_relaxed_L1L2WideResNet(args.depth, num_classes, widen_factor=args.width, droprate_init = args.droprate_init,
    		N=50000, beta_ema=args.beta_ema, weight_decay=args.weight_decay, lamba=args.lamba, alpha=args.alpha, beta=args.beta)
    elif args.reg=='SCAD':
    	model = group_relaxed_SCAD_WideResNet(args.depth, num_classes, widen_factor=args.width, droprate_init = args.droprate_init,
    		N=50000, beta_ema=args.beta_ema, weight_decay=args.weight_decay, lamba=args.lamba, alpha=3.7, beta=args.beta)
    elif args.reg=='TF1':
    	model = group_relaxed_TF1WideResNet(args.depth, num_classes, widen_factor=args.width, droprate_init = args.droprate_init,
    		N=50000, beta_ema=args.beta_ema, weight_decay=args.weight_decay, lamba=args.lamba, alpha=args.alpha, beta=args.beta)
...
def train(train_loader, model, criterion, optimizer, lr_schedule, epoch):
    ...
    if (epoch+1)%20 == 0:
        for k, layer in enumerate(layers):
            if (
                isinstance(layer, group_relaxed_L0Conv2d) or isinstance(layer, group_relaxed_L1Conv2d) or 
                isinstance(layer, group_relaxed_L1L2Conv2d) or isinstance(layer, group_relaxed_SCAD_Conv2d) or 
                isinstance(layer, group_relaxed_TF1Conv2d)
                ):
    ...
    if writer is not None:
        ...
        #writer.add_scalar('u_sparsity/epoch', u_sparsity, epoch)
...
def validate(val_loader, model, criterion, epoch):
    ...
    if writer is not None:
        ...
        #writer.add_scalar('u_sparsity/loss', u_sparsity, losses.avg)
        #writer.add_scalar('u_sparsity/test_err', u_sparsity, top1.avg)
        layers = model.layers if not args.multi_gpu else model.module.layers
        for k, layer in enumerate(layers):
